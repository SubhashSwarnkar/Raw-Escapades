...
import { Card, CardContent, CardMedia, Typography, Button, Box, IconButton, useTheme, useMediaQuery } from '@mui/material';
      ...
      <Box sx={{ padding: 2 }}>
            ...
            {!isSmallScreen && (
              <Box>
                <IconButton
                  ...
                  sx={{ mr: 1, borderRadius: '50%', background: 'rgba(255, 255, 255, 0.35)', border: '1px solid rgba(0, 0, 0, 0.23)' }}
                >
                  <ArrowBackIcon />
                </IconButton>
                <IconButton
                  onClick={() => document.querySelector('.highlighted-slick-next').click()}
                  sx={{ borderRadius: '50%', background: 'rgba(255, 255, 255, 0.35)', border: '1px solid rgba(0, 0, 0, 0.23)' }}
                >
                  <ArrowForwardIcon />
                </IconButton>
              </Box>
        ...
        <Slider {...settings} className="highlighted-slider">
          {highlightedCards.map((card, index) => (
            <Card key={index} sx={{ maxWidth: 295, margin: isSmallScreen ? '0 auto' : 'initial' }}>
              ...
              <CardContent>
                ...
                <Box display={isSmallScreen ? 'flex' : 'flex'} justifyContent="space-between" mt={2}>
                  <Button size="small" variant="contained" color="success" sx={{ borderRadius: "40px", width: '124.439px', display: 'flex', justifyContent: 'center', alignItems: 'center', mb: isSmallScreen ? 1 : 0 }}>
                    Get Details
                  </Button>
